...
def permutation_test_ad(x, y, obs_stat, n_permutations=500, random_state=42):
    """
    Permutation test for AD statistic.
    ...
    for i in range(n_permutations):
        ...
        x_perm = perm[:n_x]
        y_perm = perm[n_x:]
        perm_stats[i] = dist_ad(x_perm, y_perm)

    # p-value
    count = np.sum(perm_stats >= obs_stat)
    p_value = (count + 1) / (n_permutations + 1)

    # null distribution moments
    ad_mean_null = perm_stats.mean()
    ad_std_null = perm_stats.std(ddof=1)

    # avoid division by zero for very small segments
    if ad_std_null == 0:
        ad_zscore = 0
    else:
        ad_zscore = (obs_stat - ad_mean_null) / ad_std_null

    return p_value, ad_mean_null, ad_std_null, ad_zscore
...
def global_permutation_test_ad(chrom_sample_matrix, len_x, len_y, observed_stat, n_permutations=500, random_state=42):
    rng = np.random.default_rng(random_state)
    n_cells, n_bins = chrom_sample_matrix.shape

    # Pre-sample all random values at once
    cell_indices = rng.integers(0, n_cells, size=n_permutations)
    swap_flags = rng.random(size=n_permutations) < 0.5
    
    global_stats = []
    for j in range(n_permutations):
        l1, l2 = (len_x, len_y) if swap_flags[j] else (len_y, len_x)
        bp = rng.integers(l1, n_bins - l2 + 1)
        x_global = chrom_sample_matrix[cell_indices[j], bp - l1 : bp]
        y_global = chrom_sample_matrix[cell_indices[j], bp : bp + l2]
        global_stats.append(dist_ad(x_global, y_global))
...
def recursive_getbp(seq_data, seq_data_cell, k=3, depth=0, offset=0, n_permutations=500, alpha_local=0.05, alpha_global=0.05):
    """
    ...
    # Local permutation test to test significance within considered segment
    pval = permutation_test_ad(left_seg, right_seg, bp_dist, n_permutations)
    if pval >= alpha_local:
        # Not significant → prune this branch
        return []
    
    # Global permutation test to test significance within sample
    ...
    left_bps = recursive_getbp(left_seg, seq_data_cell, k=k, depth=depth+1, offset=offset, 
                               n_permutations=n_permutations, alpha_local=alpha_local, alpha_global=alpha_global)
...
def recursive_getbp_df(seq_data, seq_data_cell, k=3, n_permutations=500, alpha_local=0.05, alpha_global=0.05):
    """Wrapper that returns a DataFrame instead of a list"""
    results = recursive_getbp(seq_data, seq_data_cell, k=k, 
                              n_permutations=n_permutations, alpha_local=alpha_local, alpha_global=alpha_global)
...
# def recursive_getbp_save_all(seq_data, seq_data_cell, k=3, depth=0, offset=0, n_permutations=500, alpha=0.05):
#     # Base case: stop if max depth reached or sequence too short
#     if depth >= k or len(seq_data) <= 1:
#         return []
#     # Compute AD distances for this segment
...
#         return []
#     # Find best breakpoint in this segment
#     bp_local = np.argmax(dist_vector) + 1
#     bp_global = bp_local + offset
#     bp_dist = dist_vector[bp_local - 1]
#     # Split into left/right segments
#     left_seg = seq_data[:bp_local]
...
#     pval = permutation_test_ad(left_seg, right_seg, bp_dist, n_permutations)
#     if pval >= alpha:
#         # Local test failed — prune, but still record
#         return [(bp_global, bp_dist, pval, None, False)]

#     # Global permutation test
#     pval_global = global_permutation_test_ad(seq_data_cell, len(left_seg), len(right_seg), bp_dist, n_permutations)
    
#     passed = pval_global < alpha

#     # Only recurse if both tests passed
#     if passed:
#         left_bps = recursive_getbp(left_seg, seq_data_cell, k=k, depth=depth+1, offset=offset,
...
#     return left_bps + [(bp_global, bp_dist, pval, pval_global, passed)] + right_bps



# def recursive_getbp_df_save_all(seq_data, seq_data_cell, k=3, n_permutations=500, alpha=0.05):
#     results = recursive_getbp_save_all(seq_data, seq_data_cell, k=k, n_permutations=n_permutations, alpha=alpha)
#     return pd.DataFrame(results, columns=["breakpoint", "ad_dist", "p_value_local", "p_value_global", "passed"])



# Save all breakpoints alongside AD distance, local p-value and recursion depth
# def recursive_getbp_save_all(seq_data, k=3, depth=0, offset=0, n_permutations=500, alpha=0.05):
#     # Base case: stop if max depth reached or sequence too short
#     if depth >= k or len(seq_data) <= 1:
#         return []
...
#     # Always recurse regardless of test result
#     left_bps = recursive_getbp_save_all(left_seg, k=k, depth=depth+1, offset=offset,
#                                         n_permutations=n_permutations, alpha=alpha)
#     right_bps = recursive_getbp_save_all(right_seg, k=k, depth=depth+1, offset=offset+bp_local,
#                                          n_permutations=n_permutations, alpha=alpha)

#     return left_bps + [(bp_global, bp_dist, pval, passed, depth)] + right_bps


# def recursive_getbp_df_save_all(seq_data, k=3, n_permutations=500, alpha=0.05):
#     results = recursive_getbp_save_all(seq_data, k=k, n_permutations=n_permutations, alpha=alpha)
#     return pd.DataFrame(results, columns=["breakpoint", "ad_dist", "p_value_local", "passed", "depth"])



def recursive_getbp_save_all(seq_data, k=3, depth=0, offset=0,
    ...
    pval, ad_mean_null, ad_std_null, ad_z = permutation_test_ad(
        left_seg,
        right_seg,
        bp_dist,
        n_permutations
    ...
    # recursion
    left_bps = recursive_getbp_save_all(
        left_seg,
        k=k,
        depth=depth+1,
        offset=offset,
    ...
    right_bps = recursive_getbp_save_all(
        right_seg,
        k=k,
        depth=depth+1,
        offset=offset+bp_local,
        n_permutations=n_permutations,
        alpha=alpha
    )
